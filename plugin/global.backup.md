...
Domain nouns are fine; coined ones are not. Describe a thing in plain words every time.

 Resolve every reference in place: "c11, the dalo-por-hecho item", not "c11"; "the leak issue #17 describes", not
...
PR bodies:  bullet points withchange and the evidence. No section scaffolds, no "Blast radius" stamps, no pre-ticked checklists. A claim nobody verified is written as a claim.
...
Touch only what the request requires; every changed line should trace to it and those should be surgical.
