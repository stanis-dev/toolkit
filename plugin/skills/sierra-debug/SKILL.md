...
1. Download the simulation run the way `.composer/docs/agent-traces-reference.md` describes, with the target qualifier from the sierra skill's tooling.md. Make sure the replay with transcript and debug info are ready. If for any reason you cannot acess them, stop here and let me know.
2. Identify the turn that fails judge condition, or is being reported in the issue or I that I asked you to check.
3. If debugging a simulation, verify the simulation itself against the sierra skill's
   `references/sims/sim-design.md` guidance and also check that it makes sense before attempting to
...
- Agent did have the expected context and the instruction was present and clearly worded, in
  alignment with the sierra skill's `references/agent/agent-design.md` criteria:
