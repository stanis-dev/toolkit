...
- It should not have instructions on how/when to end the conversation unless it's an explicit part of the scenario being
  tested. This avoids premature persona exit, especially on agent pacing/sequencing changes.
- It should have instructions provided in a manner that allows persona to survive pacing or sequencing changes, unless
  that is explicitly what is being tested.
...
- Careful with pinning judge conditions to a specific turn. Progress indicators may appear and shift a correct answer to
  what judge will see as next turn, failing a correct scenario. Further modifications may introduces/remove intermediate
  steps that will not affect scenario's correctness. It doesn't mean you never use it - just make sure that turn pinning
  is genuinely critical for scenario correctness.
...
- Be careful about connecting expectations with tying a a condition to a specific turn. Progress Indicators can fail
  them even if agent behaviour is correct otherwise.
- Expectations must reflect conditions that are important to agent health and avoid needless strictness that could
  result in failures that are nothing but noise.

## Assessing Simulation Success

- Whenever agent phrasing is expected to change, always review how that phrasing materialises in simulation replay. Make
  sure it fits the context, register, formality level, and any other criteria it needs to obey.
