...
SYNC_SIERRA = '''#!/usr/bin/env python3
import json, os, sys, time
open(os.environ['STANDIN_LOG'], 'a').write(json.dumps({'who': 'sierra', 'args': sys.argv[1:], 't': time.time()}) + '\\n')
if sys.argv[-1] == 'pull':  # the workspace's own Studio content, when the test gives it one
    for rel, text in json.loads(os.environ.get('STANDIN_WORKSPACE') or '{}').items():
        open(os.path.join(sys.argv[2], rel), 'w').write(text)
'''
BATCH = 'stan/hip-issues-0924'


@unittest.skipUnless(lab.RERUNS, 'the scripts have no stepgit.py')
class CardSync(unittest.TestCase):
    """cardsync.py on the made-up issue: the batch merged under the card's uncommitted work, or nothing changed."""
    tearDown, bind, git, log, changes = Steps.tearDown, Steps.bind, Steps.git, Steps.log, Steps.changes

    def setUp(self):
        Steps.setUp(self)
        open(os.path.join(self.wt, 'agents/hipotecarios/node_modules/.bin/sierra'), 'w').write(SYNC_SIERRA)
        open(os.path.join(self.wt, SIM), 'w').write('a\nb\nc\nd\ne\n')
        self.git('commit', '-q', '-am', 'the card branch before the batch moved')
        self.git('branch', BATCH)
        self.git('checkout', '-q', BATCH)
        open(os.path.join(self.wt, SIM), 'w').write('A\nb\nc\nd\ne\n')
        open(os.path.join(self.wt, BLOCK), 'w').write('{"v": 2}\n')
        self.git('commit', '-q', '-am', 'another card merged into the batch')
        self.git('checkout', '-q', 'stan/hip-' + N)
        self.start = self.git('rev-parse', 'HEAD')
        base = paths.agent(self.run_dir, AG)
        os.makedirs(paths.card_dir(base, N), exist_ok=True)
        open(paths.card(base, N), 'w').write('<!-- batch: 0924 -->\n')
        json.dump({'0924': {'base': BATCH}}, open(os.path.join(base, 'batches.json'), 'w'))

    def sync(self, **env):
        r = subprocess.run([sys.executable, os.path.join(self.scripts, 'cardsync.py'), AG, N, '--pages', self.run_dir],
                           env=dict(self.env, **env), cwd=self.wt, capture_output=True, text=True, timeout=60)
    ...
    def test_takes_the_batch_with_the_work_on_top(self):
        open(os.path.join(self.wt, SIM), 'w').write('a\nb\nc\nd\nE\n')
        open(os.path.join(self.wt, 'agents/hipotecarios/simulations/new.tests.ts'), 'w').write('guard\n')
        code, out = self.sync()
        self.assertEqual(code, 0, out)
        self.assertEqual(subprocess.run(['git', '-C', self.wt, 'merge-base', '--is-ancestor', BATCH, 'HEAD']).returncode, 0)
        self.assertEqual(open(os.path.join(self.wt, SIM)).read(), 'A\nb\nc\nd\nE\n')
        self.assertEqual(open(os.path.join(self.wt, BLOCK)).read(), '{"v": 2}\n')
        self.assertEqual(self.changes(), ['?? agents/hipotecarios/simulations/new.tests.ts', 'M ' + SIM])
        self.assertEqual([r['args'][-1] for r in self.log('sierra')][-4:], ['pull', 'lint', '-y', 'pull'])
        ev = self.events()[-1]
        self.assertEqual(ev['who'], 'cardsync')
        self.assertIn('took batch 0924 in', ev['what'])
        self.assertIn('s.tests.ts', ev['what'])
        self.assertEqual(self.sync(), (0, BATCH + " is already in the card's branch\n"))
        self.assertEqual(len(self.events()), 1)

    def test_conflicting_work_leaves_everything_as_it_was(self):
        open(os.path.join(self.wt, BLOCK), 'w').write('{"v": 9}\n')
        code, out = self.sync()
        self.assertEqual(code, 3, out)
        self.assertEqual(self.git('rev-parse', 'HEAD'), self.start)
        self.assertEqual(open(os.path.join(self.wt, BLOCK)).read(), '{"v": 9}\n')
        self.assertEqual(self.changes(), ['M ' + BLOCK])
        self.assertIn('conflicts with it in ' + BLOCK, self.events()[-1]['what'])
        self.assertNotIn('push', [a for r in self.log('sierra') for a in r['args']])

    def test_a_workspace_ahead_of_the_tree_is_refused(self):
        open(os.path.join(self.wt, SIM), 'w').write('a\nb\nc\nd\nE\n')
        code, out = self.sync(STANDIN_WORKSPACE=json.dumps({'.composer/blocks/b.json': '{"studio": 1}\n'}))
        self.assertEqual(code, 2, out)
        self.assertEqual(self.git('rev-parse', 'HEAD'), self.start)
        self.assertEqual(open(os.path.join(self.wt, BLOCK)).read(), '{"v": 1}\n')
        self.assertEqual(self.changes(), ['M ' + SIM])
        self.assertIn('pull.py first', self.events()[-1]['what'])

    def test_a_generated_file_takes_the_batch_version(self):
        gen = 'agents/hipotecarios/platform-types.tsx'
        head = '// auto-generated by the Sierra SDK\n'
        open(os.path.join(self.wt, gen), 'w').write(head + 'v1\n')
        self.git('add', gen); self.git('commit', '-q', '-m', 'generated')
        self.git('checkout', '-q', BATCH); self.git('merge', '-q', '--no-edit', 'stan/hip-' + N)
        open(os.path.join(self.wt, gen), 'w').write(head + 'batch\n')
        self.git('commit', '-q', '-am', 'regenerated on the batch')
        self.git('checkout', '-q', 'stan/hip-' + N)
        open(os.path.join(self.wt, gen), 'w').write(head + 'card build\n')
        self.git('add', gen)
        code, out = self.sync()
        self.assertEqual(code, 0, out)
        self.assertEqual(open(os.path.join(self.wt, gen)).read(), head + 'batch\n')

    def test_an_archived_batch_is_left_alone(self):
        json.dump({'0924': {'base': BATCH, 'archived': '2026-09-25T00:00:00Z'}}, open(os.path.join(paths.agent(self.run_dir, AG), 'batches.json'), 'w'))
        code, out = self.sync()
        self.assertEqual(code, 5, out)
        self.assertEqual(self.git('rev-parse', 'HEAD'), self.start)

    def test_refused_while_a_step_runs(self):
        base = paths.agent(self.run_dir, AG)
        os.makedirs(os.path.dirname(paths.status(base, N, 'strategy')), exist_ok=True)
        json.dump({'state': 'working', 'pid': os.getpid()}, open(paths.status(base, N, 'strategy'), 'w'))
        code, out = self.sync()
        self.assertEqual((code, out.strip()), (5, 'not now: strategy is running'))
        self.assertEqual(self.git('rev-parse', 'HEAD'), self.start)
        self.assertEqual(self.log('sierra'), [])
