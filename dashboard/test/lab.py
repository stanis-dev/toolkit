...
copies of the real card.py, blocks.py, cardlog.py, ledger.py, stage.py, paths.py and source.py, and a fake repo. Nothing in it reaches a model,
...
def make(run):
    ...
    for f in ('card.py', 'blocks.py', 'cardlog.py', 'ledger.py', 'stage.py', 'paths.py', 'source.py') + (('stepgit.py',) if RERUNS else ()):
