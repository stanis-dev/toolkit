...
copies of the real card.py, blocks.py, ledger.py and stage.py, and a fake repo. Nothing in it reaches a model,
...
def make(run):
    ...
    for f in ('card.py', 'blocks.py', 'ledger.py', 'stage.py') + (('stepgit.py',) if RERUNS else ()):
